...
import { clearCart } from '../../store/Slices/cartPizzaSlice';
import { selectCart, selectTotalPrice, selectTotalLength } from '../../store/Slices/cartPizzaSlice/selectors';
        ...
        <div className='basket-page'>
            <div className='basket-page__wrapper'>
                <div className='basket-page__header container'>
                    ...
                    <button 
                        className='basket-page__all-delete'
                        onClick={onClearCart}
                    >
                ...
                <main className='basket-page__main container'>
                    {
                        cards.map(card => <CardBasket key={card.id} card={card}/>)
                    }
                </main>

                <div className='basket-page__footer container'>
                    <div>
                        <p className='basket-page__total-quantity'>Всего пицц: <span>{totalLength} шт.</span></p>
                        <p className='basket-page__total-price'>Сумма заказа: <span>{totalPrice} ₽</span></p>
                    </div>
                    <div>
                        <button onClick={() => navigate(-1)} className='basket-page__link-back'>
                            <svg width="8" height="14" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M7 13L1 6.93015L6.86175 1" stroke="#D3D3D3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                            <span>Вернуться назад</span>
                        </button>
                        <button className='basket-page__btn-order'>Оплатить сейчас</button>
                    </div>
                </div>                
